...
public class XenonService implements AutoCloseable {
    ...
    public boolean checkSchedulerStates() throws XenonException {
        ...
        if (recreateScheduler) {
            ...
            scheduler = Scheduler.create(schedulerConfig.getAdaptor(), schedulerConfig.getLocation(), schedulerConfig.getCredential(),
                    schedulerConfig.getProperties());
        } else {
        ...
        if (recreateFileSystem) {
            if (useSchedulerFilesystem && Scheduler.getAdaptorDescription(scheduler.getAdaptorName()).usesFileSystem()) {
                ...
                remoteFileSystem = FileSystem.create(fileSystemConfig.getAdaptor(), fileSystemConfig.getLocation(), fileSystemConfig.getCredential(),
                        fileSystemConfig.getProperties());
    ...
    public FileSystem getSourceFileSystem() throws XenonException {
        if (sourceFileSystem == null || !sourceFileSystem.isOpen()) {
            ...
            logger.debug("Creating source filesystem..." + sourceConfig.getAdaptor() + " location: " + sourceConfig.getLocation());
            logger.debug(
                    sourceConfig.getAdaptor() + " " + sourceConfig.getLocation() + " " + sourceConfig.getCredential() + " " + sourceConfig.getProperties());
            sourceFileSystem = FileSystem.create(sourceConfig.getAdaptor(), sourceConfig.getLocation(), sourceConfig.getCredential(),
    ...
    public FileSystem getTargetFileSystem() throws XenonException {
        if (targetFileSystem == null || !targetFileSystem.isOpen()) {
            ...
            logger.debug("Creating target filesystem..." + targetConfig.getAdaptor() + " location: " + targetConfig.getLocation());
            logger.debug(
                    targetConfig.getAdaptor() + " " + targetConfig.getLocation() + " " + targetConfig.getCredential() + " " + targetConfig.getProperties());
            targetFileSystem = FileSystem.create(targetConfig.getAdaptor(), targetConfig.getLocation(), targetConfig.getCredential(),
