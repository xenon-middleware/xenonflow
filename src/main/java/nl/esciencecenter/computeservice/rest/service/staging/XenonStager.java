...
public class XenonStager {
	private static final Logger logger = LoggerFactory.getLogger(XenonStager.class);
	 ...
	 * 
	 * - FileStagingObject are used to stage files from the sourceFileSystem to the targetFileSystem
	 * - StringToFileStagingObject are used to stage a literal string to the targetFileSystem
	 * 
	 * @param manifest
	 ...
	 */
	public Job doStaging(StagingManifest manifest, FileSystem sourceFileSystem, FileSystem targetFileSystem) throws XenonException{
		...
		for (StagingObject stageObject : manifest) {
			if (stageObject instanceof FileStagingObject) {
				...
				String copyId = sourceFileSystem.copy(sourcePath, targetFileSystem, targetPath, CopyMode.REPLACE, false);
	 ...
	 * - FileToStringStagingObject is used to read a remote file into a string in the job output
	 * - FileToMapStagingObject is used to read a remote file into a json object in the job output
	 * 
	 * @param manifest
	 * @throws XenonException
	 * @throws IOException
