...
import nl.esciencecenter.computeservice.rest.service.tasks.XenonWaitingTask;
...
public class XenonService {
	...
	public XenonService(ThreadPoolTaskScheduler taskScheduler) {
	...
	private void initialize() throws XenonException, IOException {
        ...
        // Re-initialize existing jobs
		
		taskScheduler.scheduleAtFixedRate(new XenonWaitingTask(this), 1000);
	}
	...
	public Scheduler getScheduler() throws XenonException {
		if (scheduler == null) {
	        ...
	        scheduler = Scheduler.create(schedulerConfig.getAdaptor(), schedulerConfig.getLocation(), schedulerConfig.getCredential(), schedulerConfig.getProperties());
		}
		return scheduler;
	}
	...
	public FileSystem getRemoteFileSystem() throws XenonException {
		if (remoteFileSystem == null || !remoteFileSystem.isOpen()) {
	        ...
	        remoteFileSystem =  FileSystem.create(fileSystemConfig.getAdaptor(), fileSystemConfig.getLocation(), fileSystemConfig.getCredential(), fileSystemConfig.getProperties());
	...
	public FileSystem getLocalFileSystem() throws XenonException {
		if (localFileSystem == null || !localFileSystem.isOpen()) {
	        ...
	        logger.debug("Creating local filesystem..."+localConfig.getAdaptor() + " location: " + localConfig.getLocation());
	        logger.debug(localConfig.getAdaptor() + " " + localConfig.getLocation() + " " + localConfig.getCredential() + " " + localConfig.getProperties());
	        localFileSystem = FileSystem.create(localConfig.getAdaptor(), localConfig.getLocation(), localConfig.getCredential(), localConfig.getProperties());
		}
	 ...
	 * 
	 * Taken from: https://stackoverflow.com/questions/7824620/logback-set-log-file-name-programmatically
	 * @param name
	 */
